...
export class PrometheusMetadataService {
  ...
  async getLabelNames(dsId: string, metric?: string): Promise<string[]> {
    const cacheKey = `${dsId}:labelNames:${metric || ''}`;
    return this.cachedFetch<string[]>(cacheKey, TTL_LABEL_NAMES, () =>
      this.fetchLabelNames(dsId, metric)
    );
  }

  async getLabelValues(dsId: string, labelName: string, selector?: string): Promise<string[]> {
    const cacheKey = `${dsId}:labelValues:${labelName}:${selector || ''}`;
    return this.cachedFetch<string[]>(cacheKey, TTL_LABEL_VALUES, () =>
      this.fetchLabelValues(dsId, labelName, selector)
    );
  }

  async getMetricMetadata(dsId: string): Promise<PrometheusMetricMetadata[]> {
    const cacheKey = `${dsId}:metricMetadata`;
    return this.cachedFetch<PrometheusMetricMetadata[]>(cacheKey, TTL_METADATA, () =>
      this.fetchMetricMetadata(dsId)
    );
  }
  ...
  private async fetchMetricNames(dsId: string): Promise<string[]> {
    const ds = await this.datasourceService.get(dsId);
    if (!ds) {
      this.logger.warn(`Datasource ${dsId} not found for getMetricNames`);
      return [];
    }
    return this.provider.getMetricNames(ds);
  }

  private async fetchLabelNames(dsId: string, metric?: string): Promise<string[]> {
    const ds = await this.datasourceService.get(dsId);
    if (!ds) {
      this.logger.warn(`Datasource ${dsId} not found for getLabelNames`);
      return [];
    }
    return this.provider.getLabelNames(ds, metric);
  }
  ...
  private async fetchLabelValues(
    ...
    return this.provider.getLabelValues(ds, labelName, selector);
  }

  private async fetchMetricMetadata(dsId: string): Promise<PrometheusMetricMetadata[]> {
    const ds = await this.datasourceService.get(dsId);
    if (!ds) {
      this.logger.warn(`Datasource ${dsId} not found for getMetricMetadata`);
      return [];
    }
    return this.provider.getMetricMetadata(ds);
  }
}
