...
export { FilterState, emptyFilters, SavedSearch };
...
/**
 * Hook that attaches real DOM resize handles to EuiBasicTable header cells.
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      if (sortField === 'datasource') {
        aVal = dsNameMap.get(a.datasourceId) ?? '';
        bVal = dsNameMap.get(b.datasourceId) ?? '';
      }
      if (sortField === 'backend') {
        aVal = a.datasourceType;
        bVal = b.datasourceType;
      }
    ...
    <>
      ...
      <div ref={searchRef} style={{ position: 'relative' }}>
        <EuiFieldSearch
          ...
          value={searchQuery}
          onChange={(e) => { onSearchChange(e.target.value); setShowSuggestions(true); setActiveSuggestion(-1); }}
