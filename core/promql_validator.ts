...
const RANGE_REQUIRED_FNS = [
  'rate', 'irate', 'increase', 'delta', 'deriv', 'changes', 'resets',
  'avg_over_time', 'min_over_time', 'max_over_time', 'sum_over_time',
  'count_over_time', 'quantile_over_time', 'last_over_time',
  'absent_over_time', 'predict_linear',
];
...
export function validatePromQL(query: string): PromQLValidationResult {
  ...
  for (let i = 0; i < query.length; i++) {
    if (pairs[query[i]]) stack.push({ char: query[i], pos: i });
      ...
      if (stack.length === 0 || stack[stack.length - 1].char !== closing[query[i]]) {
        errors.push({ message: `Unmatched '${query[i]}' at position ${i}`, severity: 'error', position: i, type: 'bracket' });
      } else { stack.pop(); }
    }
  }
  for (const s of stack) {
    errors.push({ message: `Unclosed '${s.char}' at position ${s.pos}`, severity: 'error', position: s.pos, type: 'bracket' });
  }

  // Empty label matchers
  if (/\{\s*\}/.test(query)) {
    warnings.push({ message: 'Empty label matcher — consider adding filters to reduce cardinality', severity: 'warning', type: 'label-matcher' });
  }
  ...
  for (const fn of RANGE_REQUIRED_FNS) {
    ...
    if (regex.test(query)) {
      errors.push({ message: `${fn}() requires a range vector selector [duration], e.g. ${fn}(metric[5m])`, severity: 'error', type: 'range-vector' });
    }
  }

  // High-cardinality warning
  if (/\brate\b/.test(query) && !/\brate\b.*\{[^}]*\}/.test(query)) {
    warnings.push({ message: 'Consider adding label filters to rate() to reduce cardinality', severity: 'info', type: 'cardinality' });
  }
