...
export function emptyFilters(): FilterState {
  return { status: [], severity: [], monitorType: [], healthStatus: [], labels: {}, createdBy: [], destinations: [], backend: [] };
}
...
export function matchesFilters(
  rule: {
    status: string; severity: string; monitorType: string; healthStatus: string;
    labels: Record<string, string>; createdBy: string; datasourceType: string;
...
export function filterAlerts<T extends { severity: string; state: string; labels: Record<string, string>; name: string; message?: string }>(
  alerts: T[],
  filters: { severity?: string[]; state?: string[]; labels?: Record<string, string[]>; search?: string },
): T[] {
